//! A `NativeActivity` entry point that renders a Forma frame onto Android's
//! `ANativeWindow` surface — the runtime half of the Android backend, built as a
//! cdylib (`libforma_android.so`) that an APK loads via its NativeActivity.
//!
...
//! Forma `App`, render one frame, and blit it to the window with
//! `forma_platform::backend::android::present_to_native_window`. A marker file
...
mod app {
    use forma::prelude::*;
    use forma_platform::backend::android::{
    ...
    /// Emit `msg` to logcat under the `forma` tag (CI greps `adb logcat` for it).
    fn logcat(msg: &str) {
        if let (Ok(tag), Ok(text)) = (CString::new("forma"), CString::new(msg)) {
    ...
    /// Render a Forma frame at the surface size and blit it to the window.
    unsafe extern "C" fn on_window_created(_activity: *mut ANativeActivity, window: *mut c_void) {
        unsafe {
            ...
            logcat(&format!(
                "Forma Android: window {}x{} presented={}",
                pixmap.size().width,
                pixmap.size().height,
                ok
            ));
