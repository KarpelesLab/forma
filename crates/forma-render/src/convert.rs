//! Bridges between Forma's logical-pixel geometry (`f64`) and
//! `oxideav-core`'s scene-graph geometry (`f32`).
//!
//! Keeping these conversions in one place means the rest of Forma never
//! depends on oxideav's coordinate types directly — only `forma-render` does.

use forma_geometry::{Affine, Point};
use oxideav_core::{Point as OxPoint, Transform2D};

/// Convert a Forma point to an oxideav scene point (`f64` → `f32`).
#[inline]
...
/// Convert a Forma affine transform to an oxideav `Transform2D`.
///
