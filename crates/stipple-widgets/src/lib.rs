//! Stipple's self-drawn widget library.
//!
//! Every widget is composed from the `stipple-core` [`Element`] IR and drawn by
...
use stipple_core::{Align, Anchor, Axis, BoxStyle, Cx, Element, KeyInput, OverlaySpec, ViewportId};
use stipple_geometry::{Insets, Point};
...
/// [`collect_viewports`](stipple_core::collect_viewports); until then a neutral
        ...
        match input {
            ...
            KeyInput::Copy => {
                if let Some(sel) = self.selected_text() {
                    stipple_core::set_clipboard_text(&sel);
                }
            }
            KeyInput::Cut => {
                if let Some(sel) = self.selected_text() {
            ...
            KeyInput::Paste => {
                let text = stipple_core::clipboard_text();
                if !text.is_empty() {
                    self.insert(&text);
                }
            }
...
///
/// ```
/// # use stipple_widgets::{text_editor, EditBuffer};
/// # use stipple_core::runtime::Cx;
/// # use stipple_style::Theme;
...
mod tests {
    use super::*;
    use stipple_core::{ElementKind, render_view};
    ...
    fn edit_buffer_copy_paste_round_trips_through_the_clipboard() {
        ...
        assert_eq!(stipple_core::clipboard_text(), "Clip");

        // Cut removes the selection and leaves it on the clipboard.
        b.apply(&KeyInput::SelectAll);
        b.apply(&KeyInput::Cut);
        assert_eq!(b.text(), "");
        assert_eq!(stipple_core::clipboard_text(), "ClipClip");
    }
